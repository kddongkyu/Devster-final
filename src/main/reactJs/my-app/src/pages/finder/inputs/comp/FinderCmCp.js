...
import {resetCompMember, setCm_cp, setCpChk, setCpIsValid} from "../../../../redux/compMemberSlice";
import axios from "axios";
import {useSnackbar} from "notistack";
import ToastAlert from "../../../../api/ToastAlert";
import {jwtHandleError} from "../../../../api/JwtHandleError";
import {FinderCpReset} from "../../index";

function FinderCmCp(props) {
        ...
        try {
            ...
            if (res?.status === 200) {
                if (res.data === true) {
                    ...
                    setErrorMessage(cpRegChk ? '인증 되었습니다.' : '');
                } else {
                    dispatch(setCpIsValid(false));
                    setIsInputValid(false);
                    setErrorMessage('등록되지 않은 번호입니다.');
                }
            }
        } catch (error) {
            dispatch(setCpIsValid(false));
